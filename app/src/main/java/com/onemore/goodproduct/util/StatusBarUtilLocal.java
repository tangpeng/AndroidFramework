...
//        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {//5.0  //因为5.0的系统，没有找到更改通知栏的字体，所以背景就先不处理
...
//        } else
    ...
    public static int StatusBarLightMode(Activity activity) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (MIUISetStatusBarLightMode(activity, true)) {
                ...
                result = 3;
            } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {//如果是5.0以上的系统，之前vivo5.1/5.0系统的通知栏字体都是白色，
                activity.getWindow().getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);
            }
        }
        return result;
     ...
     * @return 1:MIUUI 2:Flyme 3:android6.0
     */
    public static int StatusBarNoLightMode(Activity activity) {
        int result = 0;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
